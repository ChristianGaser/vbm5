function opts = cg_config_vbm
% Configuration file for VBM
%_______________________________________________________________________
%
% based on John Ashburners version of
% $Id: spm_config_preproc.m 424 2006-02-01 20:11:24Z john $
%
% @(#)cg_config_vbm.m	v1.03 Christian Gaser 2006/09/11

tbs = spm('tbs');
for i=1:length(tbs)
	if strcmp(lower(tbs(i).name),'vbm5')
		addpath(tbs(i).dir);
	end
end
...
%------------------------------------------------------------------------
% This field can be added to custom, by replacing a line below, with:
% custom      = branch('Custom','opts',{priors,ngaus,regtype,warpreg,warpco,biasreg,biasfwhm,samp,msk});
...
custom      = branch('Custom','custom',{priors,ngaus,regtype,warpreg,warpco,biasreg,biasfwhm,samp});
custom.help = {[...
...
'through spatial constraints of neighboring voxels (Zhang et al. IEEE TMI 2001). Neighboring voxels are ',...
'expected to have the same class labels. The prior probability of the class ',...
'and the likelihood probability of the observation is combined to estimate ',...
'the Maximum a posteriori (MAP). Prior probability can be weighted between ',...
'0 (no HMRF) and 1 (maximum HMRF for very noisy data) to cover different ',...
'levels of noise. Parts of this version are based on an implementation of ',...
'a GHMRF by Meritxell Bach Cuadra et al. (IEEE TMI 2005)']};
mrf.val    = {0.3};
...
output     = branch('Output Files','output',{grey,white,csf,biascor,vox,cleanup,mrf});
output.help = {[...
...
'modulation/* (see Figure \ref{seg2})*/. In contrast to Johns version the voxel sizes of the spatially normalised ',...
'versions can be choosen. ',...
...
write.type = 'branch';
write.name = 'VBM: Write segmentations only';
write.tag  = 'write';
write.val  = {data,output,custom};
write.prog   = @execute_write;
write.vfiles = @vfiles;
write.help = {[...
'Allows previously estimated segmentations (stored in imagename''_sn.mat'' files) ',...
...
estwrit.type = 'branch';
estwrit.name = 'VBM: Estimate + Write';
estwrit.tag  = 'estwrite';
estwrit.val  = {data,output,custom};
estwrit.prog   = @execute_estwrit;
estwrit.vfiles = @vfiles;
estwrit.help   = {[...
'Voxel based morphometry of cross-sectional data. ',...
'This function can be used for bias correcting, spatially normalising ',...
'or segmenting your data. In contrast to Johns original version this tool additionally ',...
'allows to clean up the segmentations (as used in spm2) and to apply a Hidden Markov Random Field ',...
'approach to provide spatial constraints. Furthermore, the voxel size of the segmentations ',...
'can now be choosen manually. This allows to write data with high spatial resolution which is ',...
'useful to provide small voxel size in comparison to size of gaussian smoothing filter ',...
'making the data more independent from normal distribution' ],...
'',...
[...
...
data_label = files('Data','data','c3.*\.img$',[1 Inf]);
data_label.help = {[...
'Select segmented csf image. The segmented images are used for labeling where CSF is coded with "1", ',...
'GM with "2", and WM with "3". This numeration differs from that used in SPM, but is in accordance to the commonly used labeling.'],...
'',[...
'To compute labeling you have to write all segmented images (csf, gm, and wm) before.']};

label.type = 'branch';
label.name = 'Label segmentations';
label.val  = {data_label};
label.tag  = 'label';
...
opts.name = 'VBM';
opts.tag  = 'preproc';
opts.values = {estwrit,write,label};
...
for i=1:length(p.data)
    ...
    % label segmentations
    % label is only defined if sum of all tissues is > 0.25
    mask_gwc = (sum(gwc,4) > 0.25);
    ...
    % replace c3 with l0
    V(1).fname = fullfile(pth,[nam(1:ind(1)-1) 'l0' nam(ind(1)+2:end) ext]);
...
%------------------------------------------------------------------------
function execute_estwrit(opts)
opts.custom.tpm = strvcat(opts.custom.tpm{:});
if isfield(opts.custom,'msk'),
    opts.custom.msk = strvcat(opts.custom.msk{:});
end;
for i=1:numel(opts.data),
    res           = spm_preproc(opts.data{i},opts.custom);
    [sn(i),isn]   = spm_prep2sn(res);
    [pth,nam]     = spm_fileparts(opts.data{i});
    savefields(fullfile(pth,[nam '_seg_sn.mat']),sn(i));
    savefields(fullfile(pth,[nam '_seg_inv_sn.mat']),isn);
    cg_preproc_write(sn(i),opts);
end;
return;
%------------------------------------------------------------------------

%------------------------------------------------------------------------
function execute_write(opts)
opts.custom.tpm = strvcat(opts.custom.tpm{:});
if isfield(opts.custom,'msk'),
    opts.custom.msk = strvcat(opts.custom.msk{:});
end;
for i=1:numel(opts.data),
    [pth,nam]     = spm_fileparts(opts.data{i});
    sn_name = fullfile(pth,[nam '_seg_sn.mat']);
    if exist(sn_name), sn(i) = load(sn_name);
    else
        warning(['Can''t load file ' sn_name]);    
        return    
    end
    % use selected data rather than saved data in sn
    sn(i).VF = spm_vol(char(opts.data{i}));
    cg_preproc_write(sn(i),opts);
end;
return;
%------------------------------------------------------------------------
...
%------------------------------------------------------------------------
function vf = vfiles(opts)
custom  = opts.output;
sopts = [custom.GM;custom.WM;custom.CSF];
vf    = cell(numel(opts.data),2);
for i=1:numel(opts.data),
    [pth,nam,ext,num] = spm_fileparts(opts.data{i});
    ...
    for k1=1:3,
        if sopts(k1,3),
            vf{i,j} = fullfile(pth,[  'c', num2str(k1), cnam, ext, ',1']);
            j       = j + 1;
        end;
        if sopts(k1,2),
            vf{i,j} = fullfile(pth,[ 'wc', num2str(k1), cnam, ext, ',1']);
            j       = j + 1;
        end;
        if sopts(k1,1),
            vf{i,j} = fullfile(pth,['mwc', num2str(k1), cnam, ext, ',1']);
            j       = j + 1;
        end;
    end;
end;
